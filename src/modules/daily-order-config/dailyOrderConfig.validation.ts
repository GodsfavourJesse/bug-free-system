export class DailyOrderConfigValidation {
    ...
    /**
     * Ensure the number of tasks is valid.
     */
    ...
    >(
        ...
        if (config.tasksPerDay <= 0) {
            throw new Error(
                "Tasks per day must be greater than zero.",
            );
        }
    ...
    /**
     * Ensure reward per task is valid.
     */
    ...
    /**
     * Ensure the daily reward limit is valid.
     */
    ...
    /**
     * Ensure the configuration can be
     * used for generating daily orders.
     */
    ensureUsable<
        T extends {
            isActive: boolean;
